  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="modelValue" class="fixed inset-0 z-[99999] flex items-center justify-center p-4">
        ...
        <div
          class="relative w-full max-w-lg rounded-2xl dark:bg-zinc-900 bg-white shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">

          <!-- Header -->
          <div
            class="p-5 border-b dark:border-zinc-800 border-gray-100 flex-shrink-0 flex items-start justify-between gap-3">
            ...
            <div class="overflow-y-auto scrollbar-hide p-5 space-y-4 flex-1">
              ...
              <div>
                ...
                <select v-model="form.id_provinsi" class="form-input" :disabled="auth.loadingProv">
                  ...
                  <option v-for="prov in (auth.provList as any[])" :key="prov.id_prov"
                    :value="`${prov.id_prov}-${prov.prov}`">
                    {{ prov.prov }}
                  </option>
                </select>
              </div>
              ...
              <div>
                ...
                <div class="relative">
                  ...
                  <VoiceInputButton v-if="stt.isSupported" variant="textarea"
                    :active="stt.activeField.value === 'alasan'" @click="toggleVoice('alasan')" />
                </div>
              </div>
